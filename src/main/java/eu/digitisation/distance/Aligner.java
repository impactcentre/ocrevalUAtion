...
public class Aligner {
     ...
     */
    private static String toHTML2(String first, String second) {
        int l1 = first.length();
        ...
        while (i > 0 && j > 0) {
            switch (B.get(i, j)) {
                ...
                case DELETE:
                    len = 1;
                    while (i - len > 0 && B.get(i - len, j) == EdOp.DELETE) {
                        ++len;
                    }
                   
                    s1 = first.substring(l1 - i + 1 - len, l1 - i + 1);
                    s2 = s1.replaceAll(".", "#");
                ...
                case INSERT:
                    ...
                    while (j - len > 0 && B.get(i, j - len) == EdOp.INSERT) {
                        ++len;
                    }
                  
                    s2 = second.substring(l2 - j + 1 - len, l2 - j + 1);
                    s1 = s2.replaceAll(".", "#");
                    System.out.println(len+"["+second.substring(l2-j+1- len, l2-j+2)+"]");
